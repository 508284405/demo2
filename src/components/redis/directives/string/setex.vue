<template>
    ...
    <el-form ref="ruleFormRef" :model="ruleForm" status-icon :rules="rules" label-width="120px" label-position="left"
        ...
        <el-form-item label="expirationTime" prop="expirationTime">
            <el-input style="width: 150px;" v-model="ruleForm.expirationTime" type="value" autocomplete="off" />
        </el-form-item>
...
const generate = (formEl: FormInstance | undefined) => {
    ...
    formEl.validate((valid) => {
        if (valid) {
            ruleForm.sql = `setex ${ruleForm.key} ${ruleForm.expirationTime} ${ruleForm.value}`
        } else {
            ruleForm.sql = 'error'
            return false
        }
