<template>
  <div>
   ...
   <el-tabs type="border-card" style="height: 90%;">
     <el-tab-pane label="DECR">
       <div style="display: flex;">
         <span>示例：DESC key</span>
         <Tip :content="`将键中存储的数字递减 1。如果该键不存在，则在执行操作之前将其设置为 0。如果键包含错误类型的值或包含无法表示为整数的字符串，则会返回错误。此操作限制为 64 位有符号整数。`"
     ...
     <el-tab-pane label="DECRBY">
       <DecrBy></DecrBy>
     </el-tab-pane>
     <el-tab-pane label="INCR">
       <Incr></Incr>
     </el-tab-pane>
     <el-tab-pane label="INCRBY">
       <IncrBy></IncrBy>
     </el-tab-pane>
     <el-tab-pane label="INCRBYFLOAT">
...
import DecrBy from './decrby.vue';
import Incr from './incr.vue';
import IncrBy from './incrby.vue';
import IncrByFloat from './incrbyfloat.vue';
...
const generate = (formEl: FormInstance | undefined) => {
 ...
 formEl.validate((valid) => {
   if (valid) {
     ruleForm.sql = `decr ${ruleForm.key}`
   } else {
     ruleForm.sql = 'error'
     return false
   }
