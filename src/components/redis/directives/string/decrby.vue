<template>
    <div style="display: flex;">
            <span>示例：DESCBY key decrement</span>
            <Tip :content="`将键中存储的数字递减。如果该键不存在，则在执行操作之前将其设置为 0。如果键包含错误类型的值或包含无法表示为整数的字符串，则会返回错误。此操作限制为 64 位有符号整数。`"
  ...
  const generate = (formEl: FormInstance | undefined) => {
    ...
    formEl.validate((valid) => {
      if (valid) {
        ruleForm.sql = `decrby ${ruleForm.key} ${ruleForm.decrement}`
      } else {
        ruleForm.sql = 'error'
        return false
      }
